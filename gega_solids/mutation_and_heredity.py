...
def lattice_correction(xtal_in):
    '''This function is still in development and has not yet been implemented
    '''
...
def strained_lattice_unrestricted(original_lattice):
    '''It multiplies a random strain matrix by each of the lattice vectors of a crystal unit cell
...
def atom_exchange(xtal_in,rounds):
    '''Exchanges the coordinates of two different atoms in the structure, repeating the
    process "rounds" times. 

    in: xtal_in (Molecule); The structure of interest
        rounds (Int); the number of times to repeat the process
    out: xtal_out (Molecule); The mutated structure
    '''
    xtal_out = copymol(xtal_in)
    c = 0
    while c <= rounds:
...
# x = readposcars('mis1.vasp')[0]
# y = atom_exchange(x,4)
# writeposcars([y],'mut.vasp','D')

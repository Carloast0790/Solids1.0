		...
		for j in range(i+1,l_list):
			...
			if e == True:
				if str_b in xtalist_out:
					fopen = open(log_file,'a')
					print('%s sym %11s similar to %s sym %11s, dE = %.5f dV = %.5f' %(str_a.i,sym_a,str_b.i,sym_b,dE,dV),file=fopen)
					fopen.close()
	...
	for xtala in lista:
		...
		while init == True:
			for xtalb in fh:
				...
				if equal == True:
					fopen = open(log_file,'a')
					print('%s sym %11s similar to %s sym %11s, dE = %.5f dV = %.5f' %(xtala.i,sym_a,xtalb.i,sym_b,dE,dV),file=fopen)
					fopen.close()
					init = False
